...
- [x] SSE 事件分发（message, error, close）— v1 仅处理 message 事件 (SSE 标准 default event + `event:message`); 其它自定义 event (error/close) 在 `SSEClient.Recv` 显式跳过 (sse.go L287: `frame.event != "" && frame.event != "message"` continue); endpoint 首帧已在 `parseFirstFrameLocked` 校验. docs §3.2 全文未规约 error/close 自定义 event, v1 scope-out
...
- [x] 与 Session 集成（MCP Tool 在 Session 上下文中可用）
- [x] 与 Provider 集成（MCP Tool 作为 Function 暴露给 LLM）
...
- [x] 指标按 `observability.md` §2 唯一表实现 — `internal/metrics` 极简自实现 (Counter/Gauge/Histogram + Prometheus text exposition, 不引 prometheus client_golang); Manager.SetMetrics 注入 5 个指标 `yaa_mcp_servers{status,transport}`, `yaa_mcp_tool_calls_total{server,tool,result}`, `yaa_mcp_tool_call_duration_seconds{server,tool}`, `yaa_mcp_reconnects_total{server,result}`, `yaa_mcp_tools{server}`; 接入点 publishGeneration/blob (connected + tools), markGenerationFailed/error (servers), attemptReconnect 成功/失败 (reconnects + servers), MCPToolProxy.Execute (toolCallsCounter + toolCallDurHist). label 不含 request_id/session_id/error (周知低基数); TestManagerMetricsExposeConnectEvents + TestManagerMetricsExposeReconnectErrorEvent + TestManagerMetricsToolCallCaptured 验证
- [x] 调用链追踪（span: mcp.call_tool, mcp.list_tools）— docs/mcp/observability.md §3 新增 trace 章节规约两类 span. Yaa! v1 未集成 OpenTelemetry/trace sink, 按 §3"若 Runtime 启用既有 trace sink"条件性, 由 Phase 5 统一接入. MCP 侧 §1 事件日志 + §2 指标已落地.
