...
- [x] `MemoryItem` 包含 AgentID、可选 SessionID、Layer、Key、Content、Metadata、时间、ExpiresAt、Version。
- [x] v1 只接受 `LayerLongTerm`，主键为 `(agent, layer, session, key)`。
- [x] 调用方 managed fields 必须为零；Manager 单次采样 now，Store 在事务内设置时间/Version。
...
- [x] ContentStore 完整实现 CommitPut/Get/Search/List/Delete/Clear/DeleteExpired/Count/Ping/Close。
- [x] CommitPut 在一个 SQLite 事务/Memory 写锁中提交 target + victims，失败不修改任何 item/event/index。
- [x] SQLite 使用 pure Go driver，DDL、upsert、事务、时间与 metadata 编码有集成测试。
- [x] Memory 后端遵守相同主键、排序、过期和深拷贝语义。
...
- [x] nil ExpiresAt 使用 default TTL；zero time pointer 永久；过去时间拒绝。
- [x] Get/Search/List/Count 隐藏已过期 item；cleanup 有稳定顺序、batch 和取消。
- [x] `max_items` 按 Agent 统计；fifo/ttl victim 和 tie-break 与文档一致。
- [x] 最终 count 计算覆盖新建、过期 row 恢复和未过期更新；target 排除在 victims 外，热缩后下一次 Put 原子收敛。

## 集成与失败

- [x] Agent 在 Context Build 前检索；Context 不主动访问 Memory。
- [x] Memory system message 只存在于请求副本，不写入 Session snapshot。
- [x] Session Close/Restore/Delete 不隐式摘要、Promote 或删除 Memory。
- [x] ContentStore 错误不吞掉、不写临时 fallback；向量 fallback 只由配置控制。
- [x] v1 除 Disabled 和 vector keyword fallback 外没有通用继续策略；其他 Memory 错误阻断 turn。
- [x] index degraded、Runtime Ready/Not Ready 和 Remote API 状态码符合 errors.md。
- [x] 锁顺序固定为 mutationGate → Agent keyed lock → index state；Promote 不重入 Agent lock。
...
- [x] Memory 全部文档不存在旧层级、旧写 API、未定义配置或重复 DTO。
- [x] fenced JSON/YAML 可解析，相对链接存在，Markdown fence 配对。
- [x] `git diff --check` 通过。
